...
1. What is the dimensionality of matrix $\mathbf{A}$?
2. What is the element $a_{23}$ of $\textbf{A}$?
3. What is $\textbf{A}$ + $\textbf{B}$?
4. What is $\mathbf{A}$ + $\mathbf{C}$?
5. If \(c = 2\), a scalar,  what is $c\mathbf{A}$?
...
For each of the following functions $f(x)$, find the derivative $f'(x)$ or $\frac{d}{dx}f(x)$. $c$ is a constant. 
...
1. There are 12 cards, numbered 1 to 12, and 4 cards are chosen without replacement, how many distinct possible choices are there? 
